...
fun! s:au() abort
  augroup go-coverage-coverage
    ...
    au BufWinLeave *.go call go#coverage#clear_hi(0)
    au BufWinEnter *.go
          \  for s:cov in get(s:coverage, go#coverage#go#packagepath(), [])
          \|   call go#coverage#_highlight_(0, s:cov)
          \| endfor
  augroup end
endfun
...
" if 0.
fun! go#coverage#clear_hi(winid) abort
...
fun! go#coverage#stop() abort
  ...
  for l:w in go#coverage#win#list()
    call go#coverage#clear_hi(l:w)
  endfor
endfun
...
fun! s:apply(profile) abort
  ...
  for l:w in go#coverage#win#list()
    for l:cov in get(s:coverage, go#coverage#go#packagepath(), [])
      call go#coverage#_highlight_(l:w, l:cov)
    endfor
  endfor
endfun

" Highlight the window ID as described in cov.
fun! go#coverage#_highlight_(winid, cov) abort
